...
func (repository *Publicacao) Buscar(usuarioId uint64) ([]models.Publicacao, error) {
	...
	rows, err := stmt.Query(usuarioId, usuarioId, usuarioId)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (repository *Publicacao) BuscarPublicacoesUsuario(usuarioId uint64) ([]models.Publicacao, error) {
	stmt, err := repository.db.Prepare(`
		...
		CASE 
		WHEN  t3.usuario_id THEN true
		ELSE false
		END AS curtiu 
		FROM publicacoes AS t1
		JOIN usuarios AS t2 ON t1.autor_id = t2.id
        LEFT JOIN publicacoes_curtidas AS t3 ON t3.publicacao_id = t1.id AND t3.usuario_id = ?
		WHERE t1.autor_id = ?
...
func (repository *Publicacao) BuscarPorId(id uint64) (models.Publicacao, error) {
	...
	if err = stmt.QueryRow(id, id).Scan(
		&publicacao.ID,
		&publicacao.AutorID,
		&publicacao.Titulo,
		&publicacao.Conteudo,
